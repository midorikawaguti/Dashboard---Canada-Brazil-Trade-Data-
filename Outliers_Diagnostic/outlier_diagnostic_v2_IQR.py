"""
OUTLIER GROUPING DIAGNOSTIC v2 — outlier_grouping_diagnostic.xlsx
==================================================================
...
OUTPUT_FILE = OUTPUT_DIR / "outlier_grouping_diagnostic_IQR.xlsx"
...
def compute_group_stats(df_valid: pd.DataFrame, group_cols: list) -> pd.DataFrame:
    ...
    agg["lower_fence"]  = (agg["q1"] - IQR_MULT * agg["iqr"]).round(2)
    agg["upper_fence"]  = (agg["q3"] + IQR_MULT * agg["iqr"]).round(2)
    ...
    # Merge fences back to rows — vectorised outlier detection
    ...
    std_safe         = merged["std_price"].replace(0, np.nan)
    merged["_z"]     = ((merged["_unit_price"] - merged["median_price"]) / std_safe).abs()
    ...
    # Slim flagged-rows extract — only row_id + flags (memory-safe)
    merged["_severity"] = np.where(
        merged["_out_c"], "CRITICAL",
        np.where(merged["_out_h"], "HIGH",
        np.where(merged["_out_m"], "MEDIUM", "")))
    merged["_z"] = merged["_z"].round(2)
    slim_cols = ["_row_id", "_severity", "_z", "median_price", "lower_fence", "upper_fence"]
...
def write_overview(wb, overview_df: pd.DataFrame):
    ...
    write_sub_row(ws, 2,
        ...
        f"IQR×{IQR_MULT} fences  |  Red=≥10% outlier | Orange=≥5% | Amber=≥2%",
        ncols)
...
def write_detail_sheet(wb, sheet_name: str, df_g: pd.DataFrame,
    ...
    rename = {
        "n_rows": "Valid Rows", "sufficient": "Sufficient",
        "median_price": "Median Price", "mean_price": "Mean Price",
        "std_price": "Std Dev", "cv": "CV",
        "min_price": "Min", "max_price": "Max",
        "q1": "Q1", "q3": "Q3", "iqr": "IQR",
        "lower_fence": f"Lower ({IQR_MULT}xIQR)",
        "upper_fence": f"Upper ({IQR_MULT}xIQR)",
        "n_outlier_any": "Outliers",
        "pct_outlier":   "% Outlier",
        "n_outlier_med": "MEDIUM (z≥2)",
        "n_outlier_hi":  "HIGH (z≥3)",
        "n_outlier_crit":"CRITICAL (z≥5)",
        "pct_no_outl":   "% Clean",
        "too_few":       "Status",
        "_season":       "Season",
    }
    ...
    write_sub_row(ws, 2,
        ...
        f"Red % Outlier = ≥10% | Orange = ≥5% | IQR×{IQR_MULT}", ncols)
    ...
    for ci, col in enumerate(df_out.columns, 1):
        ...
        elif col in ("Outliers","MEDIUM (z≥2)","HIGH (z≥3)","CRITICAL (z≥5)"):
            cond_map[ltr] = "outlier_count"
    ...
    for col in df_out.columns:
        ...
        elif "Fence" in col or "IQR" in col[0:3]: widths.append(15)
        elif "%" in col:                  widths.append(12)
        elif any(x in col for x in ("Outlier","MEDIUM","HIGH","CRITICAL")): widths.append(14)
...
def build_flagged_rows(df_valid: pd.DataFrame, all_flagged: dict) -> pd.DataFrame:
    ...
    for gname, slim in all_flagged.items():
        ...
        f = slim.rename(columns={
            ...
            "_z":           f"{p}_z",
            "median_price": f"{p}_median_price",
...
def write_flagged_rows_sheet(wb, flagged_df: pd.DataFrame):
    ...
    write_sub_row(ws, 2,
        ...
        "Severity: CRITICAL=|z|>=5  HIGH=|z|>=3  MEDIUM=|z|>=2", ncols)
...
def main(input_path: Path):
    S = "=" * 65
    print(f"\n{S}\n  OUTLIER GROUPING DIAGNOSTIC v2 (fast)\n{S}\n")
