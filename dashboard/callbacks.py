...
def register_callbacks(app):
    ...
    def sync_hs2_chart_to_dropdown(click_data, current_hs2):
        """
        Clicking a bar in the HS2 share chart sets the HS2 dropdown.
        Clicking the same bar again clears the filter (toggle).
        """
    ...
    def update_commodity_detail(selected_commodity, period_range,
        ...
        if has_price and kpis['yoy_price'] is not None:
            yoy_str   = f'+{kpis["yoy_price"]:.1f}%' if kpis['yoy_price'] >= 0 else f'{kpis["yoy_price"]:.1f}%'
        ...
        kpi_tiles = [
            html.Div(style=KPI_STYLE_BOX, children=[
                ...
                html.P('Total Trade Value', style={'color': TEXT_GRAY, 'fontSize': '12px', 'margin': '4px 0 0 0'}),
            ]),
            html.Div(style=KPI_STYLE_BOX, children=[
                html.H2(f'{kpis["total_quantity"]:,.0f}', style=KPI_STYLE_VALUE),
                html.P(f'Total Quantity ({kpis["unit"]})', style={'color': TEXT_GRAY, 'fontSize': '12px', 'margin': '4px 0 0 0'}),
            ]),
        ]
        if has_price:
            kpi_tiles.append(html.Div(style=KPI_STYLE_BOX, children=[
                html.H2(avg_price_str, style=KPI_STYLE_VALUE),
                html.P('Avg Price per Unit', style={'color': TEXT_GRAY, 'fontSize': '12px', 'margin': '4px 0 0 0'}),
            ]))
            if yoy_str:
                kpi_tiles.append(html.Div(style=KPI_STYLE_BOX, children=[
                    html.H2(yoy_str, style={**KPI_STYLE_VALUE, 'color': yoy_color}),
                    html.P('YoY Price Change', style={'color': TEXT_GRAY, 'fontSize': '12px', 'margin': '4px 0 0 0'}),
                    html.P('vs same period prior year', style={'color': TEXT_GRAY, 'fontSize': '10px', 'fontStyle': 'italic', 'margin': '2px 0 0 0'}),
                ]))
        else:
            kpi_tiles.append(html.Div(style={**KPI_STYLE_BOX, 'borderLeft': '3px solid #FFAA00', 'backgroundColor': '#FFFBF0'}, children=[
                html.P('⚠️  Price data unavailable', style={'color': '#AA6600', 'fontSize': '12px', 'fontWeight': 'bold', 'margin': '0 0 4px 0'}),
                html.P('Quantity = 0 for all records — price per unit cannot be computed', style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '0'}),
            ]))
        ...
        if has_price:
            price_row = html.Div(
                ...
                children=[
                    html.Div(style={**STYLE_CHART_ITEM, 'flex': '1'}, children=[
                        ...
                        dcc.Graph(figure=build_price_histogram(filtered, selected_commodity), config={'displayModeBar': False}),
                    ]),
                    html.Div(style={**STYLE_CHART_ITEM, 'flex': '1'}, children=[
                        html.H4('Avg Price Over Time', style=FIGURE_TITLE),
                        html.P('Monthly median price per unit', style=FIGURE_DESCRIPTION),
                        dcc.Graph(figure=build_price_over_time(filtered, selected_commodity), config={'displayModeBar': False}),
                    ]),
                    html.Div(style={**STYLE_CHART_ITEM, 'flex': '1'}, children=[
                        html.H4('Seasonality', style=FIGURE_TITLE),
                        html.P('Avg monthly trade value across years', style=FIGURE_DESCRIPTION),
                        dcc.Graph(figure=build_seasonality_chart(filtered, selected_commodity), config={'displayModeBar': False}),
                    ]),
                ]
            )
        else:
            price_row = html.Div(
                ...
                children=[
                    html.P('⚠️  Price charts not available', style={'color': '#AA6600', 'fontWeight': 'bold', 'margin': '0 0 4px 0', 'fontSize': '13px'}),
                    html.P('Price distribution, avg price over time and seasonality require Quantity > 0 to compute price per unit. All records for this commodity have Quantity = 0.',
                           style={'color': TEXT_GRAY, 'fontSize': '12px', 'margin': '0'}),
                ]
            )

        # ── Destinations row — only show available directions ──────────────────
        dest_children = []
        if has_exports:
            dest_children.append(html.Div(style={**STYLE_CHART_ITEM, 'flex': '1'}, children=[
                html.H4('Top Export Destinations', style=FIGURE_TITLE),
                html.P('Countries buying this commodity from Canada', style=FIGURE_DESCRIPTION),
                dcc.Graph(figure=build_commodity_export_destinations(filtered, selected_commodity), config={'displayModeBar': False}),
            ]))
        if has_imports:
            dest_children.append(html.Div(style={**STYLE_CHART_ITEM, 'flex': '1'}, children=[
                html.H4('Top Import Origins', style=FIGURE_TITLE),
                html.P('Countries supplying this commodity to Canada', style=FIGURE_DESCRIPTION),
                dcc.Graph(figure=build_commodity_import_origins(filtered, selected_commodity), config={'displayModeBar': False}),
            ]))
        ...
        blank_u     = len(sub[sub['Unit of measure'] == 'Blank']) if 'Unit of measure' in sub.columns else 0
        countries_n = sub['Country'].nunique() if 'Country' in sub.columns else 0
        ...
        data_quality = html.Div(
            ...
            children=[
                ...
                html.H4('📊  Data Quality', style={'color': BLUE_ACCENT, 'fontSize': '14px', 'margin': '0 0 12px 0'}),
                html.Div(style={'display': 'flex', 'gap': '12px', 'flexWrap': 'wrap'}, children=[
                    html.Div(style={**KPI_STYLE_BOX, 'minWidth': '130px'}, children=[
                        html.H3(f'{total_r:,}', style={**KPI_STYLE_VALUE, 'fontSize': '22px'}),
                        html.P('Total Rows', style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '4px 0 0 0'}),
                    ]),
                    html.Div(style={**KPI_STYLE_BOX, 'minWidth': '130px',
                                    'borderLeft': f'3px solid {"#1A4731" if valid_r == total_r else "#FFAA00"}'}, children=[
                        html.H3(f'{valid_r:,}', style={**KPI_STYLE_VALUE, 'fontSize': '22px', 'color': '#1A4731' if valid_r == total_r else '#AA6600'}),
                        html.P(f'Valid Rows ({valid_r/total_r*100:.0f}% of total)' if total_r > 0 else 'Valid Rows',
                               style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '4px 0 0 0'}),
                        html.P('Qty > 0, price computable', style={'color': TEXT_GRAY, 'fontSize': '10px', 'fontStyle': 'italic', 'margin': '2px 0 0 0'}),
                    ]),
                    # Zero quantity — only show if there are any
                    *([html.Div(style={**KPI_STYLE_BOX, 'minWidth': '130px',
                                    'borderLeft': '3px solid #C00000'}, children=[
                        html.H3(f'{zero_r:,}', style={**KPI_STYLE_VALUE, 'fontSize': '22px', 'color': '#C00000'}),
                        html.P('Zero Quantity', style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '4px 0 0 0'}),
                        html.P('Cannot compute price/unit', style={'color': TEXT_GRAY, 'fontSize': '10px', 'fontStyle': 'italic', 'margin': '2px 0 0 0'}),
                    ])] if zero_r > 0 else []),
                    # Blank unit — only show if there are any
                    *([html.Div(style={**KPI_STYLE_BOX, 'minWidth': '130px'}, children=[
                        html.H3(f'{blank_u:,}', style={**KPI_STYLE_VALUE, 'fontSize': '22px'}),
                        html.P('Blank Unit', style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '4px 0 0 0'}),
                        html.P('No unit of measure', style={'color': TEXT_GRAY, 'fontSize': '10px', 'fontStyle': 'italic', 'margin': '2px 0 0 0'}),
                    ])] if blank_u > 0 else []),
                    html.Div(style={**KPI_STYLE_BOX, 'minWidth': '130px'}, children=[
                        html.H3(f'{countries_n:,}', style={**KPI_STYLE_VALUE, 'fontSize': '22px', 'color': BLUE_ACCENT}),
                        html.P('Countries', style={'color': TEXT_GRAY, 'fontSize': '11px', 'margin': '4px 0 0 0'}),
                    ]),
                ])
            ]
        )
        ...
        return html.Div(
            ...
            children=[
                ...
                html.Div(style={'display': 'flex', 'justifyContent': 'space-between',
                    ...
                    html.Div(children=[
                        html.H4(f'🔬  {short_name}', style={'margin': '0 0 4px 0', 'color': BLUE_ACCENT, 'fontSize': '15px', 'fontWeight': 'bold'}),
                        html.P('Price distribution, trend, seasonality and trade partners',
                               style={'margin': '0', 'color': TEXT_GRAY, 'fontSize': '12px', 'fontStyle': 'italic'}),
                    ]),
                    html.Button('📊  Data Quality', id='data-quality-toggle', n_clicks=0,
                                style={'backgroundColor': WHITE, 'border': f'1px solid {BLUE_ACCENT}',
                                       'borderRadius': '6px', 'color': BLUE_ACCENT, 'cursor': 'pointer',
                                       'fontSize': '12px', 'fontWeight': 'bold', 'padding': '6px 14px'}),
                ]),

                # Trade direction banner
                html.Div(style={'backgroundColor': dir_bg, 'border': f'1px solid {dir_color}',
                ...
                # KPI tiles
                html.Div(style={**KPI_STYLE_ROW, 'margin': '0 0 20px 0'}, children=kpi_tiles),
    ...
    def toggle_data_quality(n_clicks):
        from .styles import WHITE, BLUE_ACCENT
